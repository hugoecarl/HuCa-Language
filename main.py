...
class BinOp(Node):
    ...
    def evaluate(self, SymbolTable):
        if self.value in ['+', '-', '*', '/', 'greater', 'less', 'or', 'and'] and (self.children[0].evaluate(SymbolTable)[1] == str or self.children[1].evaluate(SymbolTable)[1] == str):
...
class Tokenizer:
    ...
    reserved_words = ['show', 'less', 'less_eq', 'greater', 'greater_eq', 'not', '@', '?', '?.', 'or', 'and', 'read', 'true', 'false', 'block', 'return']
    ...
    def selectNext(self):
        ...
        if self.origin[self.position] == " ":
            ...
            self.position += 1
            return self.selectNext()
        elif self.origin[self.position].isalpha() or self.origin[self.position] == '?' or self.origin[self.position] == '@':
            buffer = ''
            while self.origin[self.position].isalpha() or self.origin[self.position] == '_' or self.origin[self.position].isnumeric() or self.origin[self.position] == '?' or self.origin[self.position] == '@' or self.origin[self.position] == '.': 
            ...
            self.position += 1
            buf = ""
            if self.origin[self.position] == "I" or self.origin[self.position] == "E":
                buf += self.origin[self.position]
                self.position += 1
                if self.origin[self.position] == "N" or self.origin[self.position] == "N":
                    buf += self.origin[self.position]
                    self.position += 1
                    if self.origin[self.position] == "I" or self.origin[self.position] == "D":
                        buf += self.origin[self.position]
                        self.position += 1
                        if buf == "END":
                            self.actual = Token('EndProgram', None)
                            return self.actual
                        if self.origin[self.position] == "T":
                            self.position += 1
                            self.actual = Token('StartProgram', None)
                            return self.actual
                raise Exception("Wrong Start or End Program Token")
        elif self.origin[self.position] == "|":
            ...
            self.position += 1
            return self.actual
        elif self.origin[self.position] == "concat":
            self.actual = Token('String', 'concat')
            self.position += 1
            return self.actual
        elif self.origin[self.position] == "<":
...
class Parser:  
    ...
    def parseBlock():
        ...
        if Parser.nex.value == '=>':
            ...
            raise Exception("Error Parsing the program")
    ...
    def parseRelExpression():
        ...
        while Parser.nex.value in ['eqto','greater','less', 'greater_eq', 'less_eq']:
            if Parser.nex.value == 'eqto':
                    Parser.nex = Parser.tokens.selectNext()            
                    result = BinOp('eqto', [result, Parser.parseExpression()])
            elif Parser.nex.value == 'less':
                Parser.nex = Parser.tokens.selectNext()            
                result = BinOp('less', [result, Parser.parseExpression()])            
            elif Parser.nex.value == 'greater':
                Parser.nex = Parser.tokens.selectNext()            
                result = BinOp('greater', [result, Parser.parseExpression()])
            elif Parser.nex.value == 'less_eq':
                Parser.nex = Parser.tokens.selectNext()            
                result = BinOp('less_eq', [result, Parser.parseExpression()])            
            elif Parser.nex.value == 'greater_eq':
                Parser.nex = Parser.tokens.selectNext()            
                result = BinOp('greater_eq', [result, Parser.parseExpression()])
        return result               
    ...
    def parseExpression():    
        ...
        while Parser.nex.value in ['+','-','or','concat']:
            if Parser.nex.value == '+':
                Parser.nex = Parser.tokens.selectNext()            
    ...
    def parseFactor():      
        ...
        if Parser.nex.type == 'Num':
            ...
            Parser.nex = Parser.tokens.selectNext()
            result = UnOp('-', [Parser.parseFactor(), None])
        elif Parser.nex.value == 'not':
            Parser.nex = Parser.tokens.selectNext()
            result = UnOp('not', [Parser.parseFactor(), None])
        elif Parser.nex.value == '(':
            Parser.nex = Parser.tokens.selectNext()
            ...
            if Parser.nex.value == 'read':
                ...
                if Parser.nex.value == '<':
                    ...
                    if Parser.nex.value == '>':
                        ...
                        raise Exception('Wrong call read<> missing ">"')
                else:
                    raise Exception('Wrong call read<> missing "<"')
